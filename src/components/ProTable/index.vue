...
<template>
    <!-- 查询表单 -->
    <SearchForm v-show="isShowSearch" :search="_search" :reset="_reset" :columns="searchColumns" :search-param="searchParam" :search-col="searchCol" :is-collapsed="isCollapsed" />
    ...
    <div class="card table-main">
        ...
        <div class="table-header">
            ...
            <div v-if="toolButton || $slots.tableHeaderRight" class="header-button-ri">
                <slot name="tableHeaderRight" :selected-list="selectedList" :selected-list-ids="selectedListIds" :is-selected="isSelected" />
        ...
        <!-- 表格主体 -->
        <slot name="table" :loading="loading" :id="uuid" :colums="tableColumns" :data="processTableData" :border="border" :row-key="rowKey" :selectionChange="selectionChange" :sortChange="sortChange">
            <el-table ref="tableRef" v-bind="$attrs" v-loading="loading" :id="uuid" :data="processTableData" :border="border" :row-key="rowKey" @selection-change="selectionChange" @sort-change="sortChange">
                <!-- 默认插槽 -->
                <slot />
        ...
        <slot name="pagination" :pageable="pageable" :handle-size-chang="handleSizeChange" :handle-current-change="handleCurrentChange">
            <Pagination v-if="pagination && pageable.total" :page-sizes="pageSizes" :pageable="pageable" :handle-size-change="handleSizeChange" :handle-current-change="handleCurrentChange" />
        </slot>
    </div>
    <!-- 列设置 -->
...
export interface ProTableProps {
    ...
    requestApi?: (params: any, sortParam?: any) => Promise<any>; // 请求表格数据的 api ==> 非必传
...
const props = withDefaults(defineProps<ProTableProps>(), {
    columns: () => [],
    requestAuto: true,
    pageSizes: () => [10, 15, 25, 50, 80, 100, 200],
    pagination: true,
    initParam: {},
    border: true,
    toolButton: true,
...
// table 实例
const tableRef = inject("proTableRef", ref<InstanceType<typeof ElTable>>());
